...
export function CartProvider({ children }) {
  ...
  const [deliveryMethod, setDeliveryMethodState] = useState('standard')
  ...
  const ensureCart = useCallback(async () => {
    ...
    const { data: created, error: insertError } = await supabase
      .from('carts')
      .insert({ customer_id: user.id, delivery_method: 'standard' })
      .select('id,delivery_method')
      .single()
    ...
    setDeliveryMethodState(created.delivery_method || 'standard')
    return created.id
  }, [user?.id])
  ...
  const refreshCart = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      setItems(rawItems.map(item => {
        ...
        const isAvailable = Boolean(product?.is_active && store?.status === 'active' && !variantRequiredMissing && availableStock > 0)
        return {
          ...item,
          product,
          variant,
  ...
  const addItem = useCallback(async (productId, productVariantId = null, quantity = 1, snapshot = {}) => {
    ...
    const availableStock = Number.isFinite(knownStock) && knownStock >= 0 ? knownStock : await getAvailableStock(productId, productVariantId)
  ...
  const setDeliveryMethod = useCallback(async (method) => {
  ...
  const deliveryFeeCdf = deliveryOption(deliveryMethod).feeCdf
  ...
  const value = useMemo(() => ({
    ...
    clearCart,
    refreshCart,
  }), [cartId, items, loading, count, total, hasIssues, deliveryMethod, deliveryFeeCdf, setDeliveryMethod, addItem, updateQuantity, removeItem, clearCart, refreshCart])
