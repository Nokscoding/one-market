  ...
  async function loadDashboard() {
    ...
    try {
      const [orderResult, addressResult, notificationResult, ticketResult] = await Promise.all([
        ...
        supabase.from('addresses').select('id,customer_id,address_type,label,full_name,phone,whatsapp_phone,country_code,address_line1,address_line2,district,city,state_region,instructions,is_default,building,apartment,landmark,latitude,longitude,created_at').eq('customer_id', user.id).order('is_default', { ascending: false }).order('created_at', { ascending: false }),
