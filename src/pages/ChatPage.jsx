 ...
 * BUT: conversation client ↔ vendeur pour une sous-commande.
 * SUPABASE: conversations, seller_orders, stores, order_items, messages + Realtime.
 */
...
export default function ChatPage() {
  ...
  useEffect(() => {
    async function load() {
      const { data: c } = await supabase.from('conversations').select('*').eq('id', id).maybeSingle(); setConversation(c || null)
      if (c) {
        const { data: so } = await supabase.from('seller_orders').select('*').eq('id', c.seller_order_id).maybeSingle(); setSuborder(so || null)
        if (so) {
  ...
  async function send(e) {
    e.preventDefault(); const content = text.trim(); if (!content) return
  ...
  return <main className="chat-page section-shell"><div className="chat-shell"><header className="chat-head"><div>{store?.logo_url ? <img src={store.logo_url} alt="" /> : <div className="chat-avatar">{store?.name?.slice(0,2).toUpperCase() || 'OM'}</div>}<div><span>Boutique OneMarket</span><h1>{store?.name || 'Vendeur'}</h1></div></div><div><span>Commande boutique</span><strong>{suborder?.seller_order_number}</strong></div></header><div className="chat-items">{items.map(item => <button key={item.id} className={selectedItem === item.id ? 'active' : ''} onClick={() => setSelectedItem(selectedItem === item.id ? '' : item.id)}>{item.product_image_url ? <img src={item.product_image_url} alt="" /> : <span className="chat-item-placeholder">OM</span>}<span>{item.product_name}</span><strong>{money(item.line_total, suborder?.currency)}</strong></button>)}</div><div className="chat-notice">Utilisez ce chat pour convenir du paiement et de la livraison. Le vendeur confirmera ensuite la réception du paiement.</div><div className="messages">{messages.length ? messages.map(m => { const linked = m.order_item_id ? itemMap[m.order_item_id] : null; return <div className={`message ${m.sender_id === user.id ? 'mine' : ''}`} key={m.id}>{linked && <span className="message-product">À propos de : {linked.product_name}</span>}<p>{m.content}</p><time>{dateTime(m.created_at)}</time></div>}) : <div className="chat-empty"><strong>La conversation commence ici.</strong><span>Écrivez au vendeur pour organiser le paiement et la livraison.</span></div>}<div ref={endRef}/></div><form className="chat-compose" onSubmit={send}><div className="compose-context">{selectedItem ? `Article : ${itemMap[selectedItem]?.product_name}` : 'Message général à la boutique'}</div><div><textarea rows="1" value={text} onChange={e => setText(e.target.value)} placeholder="Écrire un message…" /><button className="send-button" disabled={sending || !text.trim()}><Send size={18} /></button></div>{error && <span className="form-error">{error}</span>}</form></div></main>
}
