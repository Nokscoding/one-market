...
export default function StorePage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      if (!active) return
      setStore(s || null); setProducts([]); setCategoryName('')

      if (s) {
        const categoryPromise = s.primary_category_id
          ? supabase.from('categories').select('name').eq('id', s.primary_category_id).maybeSingle()
      ...
      <section className="store-hero">
        ...
        <div className="section-shell store-profile store-profile--final">
          ...
          <div className="store-profile-copy">
            <span className="eyebrow">Boutique One Market vérifiée</span>
            <h1>{store.name}</h1>
            <p>{store.description || 'Boutique sélectionnée sur One Market.'}</p>
