    ...
    <main className="section-shell cart-final-page">
      ...
      <div className="cart-final-layout">
        <div className="cart-final-groups">
          ...
          <section className="cart-plus-card"><div className="cart-plus-main"><div className="cart-plus-icon"><Crown size={22}/></div><div><span>One Market Plus</span><strong>Commandez souvent, dépensez moins en livraison.</strong><p>Formules à partir de {money(ONE_MARKET_PLUS.plans[0].priceUsd, ONE_MARKET_PLUS.plans[0].currency)} / mois.</p></div><button type="button" onClick={() => setPlusOpen(value => !value)}>{plusOpen ? 'Masquer' : 'Découvrir'}</button></div>{plusOpen && <div className="cart-plus-details"><div className="cart-plus-plans">{ONE_MARKET_PLUS.plans.map(plan => <span key={plan.code}><strong>{plan.label}</strong><b>{money(plan.priceUsd, plan.currency)}</b><small>{plan.duration}</small></span>)}</div><ul>{ONE_MARKET_PLUS.benefits.map(benefit => <li key={benefit}><CheckCircle2Fallback/>{benefit}</li>)}</ul></div>}</section>
        </div>
        ...
        <aside className="cart-final-summary">
          ...
          <div className="cart-marketplace-benefits"><div><Banknote size={18}/><span><strong>Paiement à la livraison</strong><small>Payez directement le livreur à la réception. Mobile Money arrive bientôt.</small></span></div><div><Truck size={18}/><span><strong>Livraison RDC</strong><small>Normale ou express selon votre choix.</small></span></div><div><Store size={18}/><span><strong>Panier multi-boutiques</strong><small>Chaque boutique prépare uniquement ses articles.</small></span></div></div>
        </aside>
      </div>
    </main>
  )
