...
export default function CartPage() {
  ...
  if (!items.length) {
    return <main className="section-shell page-space cart-empty-page"><EmptyState title="Votre panier est vide" text="Ajoutez des produits de plusieurs boutiques. One Market organisera automatiquement votre commande par vendeur." action={<Link className="button primary" to="/catalog">Découvrir les produits</Link>}/></main>
  }
  ...
  return (
    <main className="section-shell cart-final-page">
      <div className="cart-final-heading">
        <div><span className="eyebrow">Panier One Market</span><h1>Votre panier</h1><p>{count} article{count > 1 ? 's' : ''} · {groups.length} boutique{groups.length > 1 ? 's' : ''}</p></div>
        <div className="cart-heading-actions"><Link to="/catalog">Continuer mes achats</Link><button className={confirmClear ? 'is-confirm' : ''} onClick={emptyCart}>{confirmClear ? 'Confirmer : vider le panier' : 'Vider le panier'}</button></div>
      </div>
      ...
      <div className="cart-final-layout">
        <div className="cart-final-groups">
          ...
          <section className="cart-delivery-card">
            <div className="cart-delivery-head"><Truck size={22}/><div><span className="eyebrow">Livraison</span><h2>Choisissez votre livraison</h2><p>Le tarif est revérifié côté serveur lors de la création de la commande.</p></div></div>
            <div className="cart-delivery-options">{DELIVERY_OPTIONS.map(option => <button type="button" disabled={deliverySaving} key={option.code} className={deliveryMethod === option.code ? 'active' : ''} onClick={() => chooseDelivery(option.code)}><span className="cart-delivery-radio">{deliveryMethod === option.code && <i/>}</span><div><strong>{option.label}</strong><small>{option.description}</small></div><b>{cdf(option.feeCdf)}</b>{option.code === 'express' && <em><Zap size={13}/> Prioritaire</em>}</button>)}</div>
          </section>

          <section className="cart-plus-card">
            <div className="cart-plus-main"><div className="cart-plus-icon"><Crown size={22}/></div><div><span>One Market Plus · proposition</span><strong>Tu commandes souvent ? Réduis tes frais de livraison.</strong><p>À partir de {cdf(ONE_MARKET_PLUS.plans[0].priceCdf)} / mois. Aucun abonnement n’est activé ou facturé pour le moment.</p></div><button type="button" onClick={() => setPlusOpen(value => !value)}>{plusOpen ? 'Masquer' : 'Découvrir'}</button></div>
            {plusOpen && <div className="cart-plus-details"><div className="cart-plus-plans">{ONE_MARKET_PLUS.plans.map(plan => <span key={plan.code}><strong>{plan.label}</strong><b>{cdf(plan.priceCdf)}</b><small>{plan.duration}</small></span>)}</div><ul>{ONE_MARKET_PLUS.benefits.map(benefit => <li key={benefit}><CheckCircle2Fallback/>{benefit}</li>)}</ul><small className="cart-plus-note">Offre préparée pour une activation future lorsqu’un vrai processus de paiement/abonnement sera disponible.</small></div>}
          </section>
        </div>

        <aside className="cart-final-summary">
          <div className="cart-summary-card"><span className="cart-summary-kicker">Résumé de la commande</span><h2>{count} article{count > 1 ? 's' : ''}</h2><div className="cart-summary-row"><span>Sous-total articles</span><strong>{money(total, 'USD')}</strong></div><div className="cart-summary-row"><span>{deliveryMethod === 'express' ? 'Livraison express' : 'Livraison normale'}</span><strong>{cdf(deliveryFeeCdf)}</strong></div><div className="cart-summary-row"><span>Mode de paiement</span><b>Paiement à la livraison</b></div><div className="cart-summary-total"><span>Total produits</span><strong>{money(total, 'USD')}</strong></div><div className="cart-summary-cdf"><span>+ Livraison</span><strong>{cdf(deliveryFeeCdf)}</strong></div><small>Les produits sont facturés en USD et la livraison en FC. One Market n’applique aucune conversion USD/CDF arbitraire.</small>{hasIssues ? <button className="button primary full" disabled>Corrigez le panier avant de continuer</button> : <Link className="button primary full" to="/checkout">Passer au checkout <ArrowRight size={18}/></Link>}<div className="cart-summary-security"><ShieldCheck size={18}/><span><strong>Stock et livraison vérifiés</strong><small>Les deux sont revérifiés par Supabase au moment de commander.</small></span></div></div>
          <div className="cart-marketplace-benefits"><div><Banknote size={18}/><span><strong>Paiement à la livraison</strong><small>Payez à la réception pour la V1.</small></span></div><div><Truck size={18}/><span><strong>Livraison RDC</strong><small>Normale ou express selon votre choix.</small></span></div><div><Store size={18}/><span><strong>Panier multi-boutiques</strong><small>Chaque vendeur reçoit uniquement ses articles.</small></span></div></div>
        </aside>
      </div>
    </main>
  )
