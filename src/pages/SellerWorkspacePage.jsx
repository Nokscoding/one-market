        ...
        {!workspaceLoading && tab === 'orders' && <section className="seller-panel seller-orders-panel">
          ...
          <div className="seller-orders-list">{orders.map(order => {
            ...
            return <article className="seller-order-detail-card" key={order.id}>
              ...
              <footer>{action && <button className="button primary" disabled={actionSaving === order.id} onClick={() => runOrderAction(order, action.action)}>{actionSaving === order.id ? 'Mise à jour…' : action.label}</button>}{['pending','confirmed'].includes(order.status) && <button className="button secondary danger" disabled={actionSaving === order.id} onClick={() => setRefusal({ order, reason: '' })}>Refuser</button>}</footer>
            </article>
