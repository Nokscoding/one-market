...
const EMPTY_STORE = { name: '', city: 'Lubumbashi', description: '', logo_url: '', banner_url: '' }
const EMPTY_PRODUCT = {
...
export default function SellerPage() {
    ...
    ;(async () => {
      ...
      if (!isSeller) {
        ...
        if (active) {
          ...
          setApplicationForm(current => ({ ...current, phone: profile?.phone || current.phone }))
        }
        return
      }
      ...
      .catch(error => {
        if (active) setFeedback(error?.message || 'Impossible de charger l’espace vendeur.')
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    setStoreForm({
      ...
      city: selectedStore.city || 'Lubumbashi',
    ...
    ;(async () => {
      ...
      const [imagesResult, itemsResult, parentsResult] = await Promise.all([
        ...
        sellerOrderIds.length
          ? supabase.from('order_items').select('*').in('seller_order_id', sellerOrderIds).order('created_at')
          : Promise.resolve({ data: [] }),
        orderIds.length
          ? supabase.from('orders').select('id,order_number,shipping_snapshot,customer_note,payment_method,payment_status,created_at').in('id', orderIds)
          : Promise.resolve({ data: [] }),
      ])
      if (!active) return
      ...
      .catch(error => {
        if (active) setFeedback(error?.message || 'Impossible de charger les données vendeur.')
      })
      .finally(() => {
  ...
  async function createProduct(event) {
    ...
    if (productForm.image_url.trim()) {
      ...
      if (!imageResult.error) imageUrl = imageResult.data?.secure_url || ''
    }
  ...
  async function refuseOrder(order) {
    ...
    const { data, error } = await supabase
      .from('seller_orders')
      .update({ status: 'refused', refusal_reason: reason.trim(), updated_at: new Date().toISOString() })
      .eq('id', order.id)
      .select('*')
      .single()
    if (error) return setFeedback(error.message)
          ...
          {application ? (
            ...
            <form onSubmit={submitApplication} className="seller-join-form">
              ...
              <div className="seller-form-row">
                <label>Téléphone<input value={applicationForm.phone} onChange={event => setApplicationForm({ ...applicationForm, phone: event.target.value })} placeholder="+243…"/></label>
                <label>Ville<input value={applicationForm.city} onChange={event => setApplicationForm({ ...applicationForm, city: event.target.value })}/></label>
              </div>
    ...
    return (
      <main className="section-shell seller-setup-page">
        <section className="seller-setup-card">
          ...
          <h1>Crée ta boutique</h1>
          <p>Ton compte vendeur est actif. Configure maintenant la boutique visible par les clients.</p>
    ...
    <main className="seller-workspace">
      <aside className="seller-sidebar">
        ...
        {isAdmin && stores.length > 1 && (
          <label className="seller-store-selector">Boutique
            <select value={selectedStoreId} onChange={event => setSelectedStoreId(event.target.value)}>
              {stores.map(store => <option key={store.id} value={store.id}>{store.name}</option>)}
            </select>
          </label>
        )}
        <div className="seller-current-store">
          <SmartImage src={selectedStore?.logo_url} fallback={(selectedStore?.name || 'OM').slice(0,2).toUpperCase()} fit="contain" widthHint={100}/>
          <div><strong>{selectedStore?.name}</strong><span>{selectedStore?.city || 'RDC'}</span></div>
        </div>
        <nav>
      ...
      <section className="seller-main">
        <header className="seller-topbar">
          <div>
            ...
            <h1>{tab === 'overview' ? 'Tableau de bord' : tab === 'products' ? 'Produits & stock' : tab === 'orders' ? 'Commandes' : 'Ma boutique'}</h1>
          </div>
            ...
            <div className="seller-dashboard-grid">
              <section className="seller-panel">
                ...
                <div className="seller-order-mini-list">
                  {orders.slice(0,5).map(order => (
                    <button key={order.id} onClick={() => changeTab('orders')}>
                      <span className="seller-order-icon"><ShoppingBag size={18}/></span>
                      <div><strong>{order.seller_order_number}</strong><small>{formatDate(order.created_at)}</small></div>
                      <b className={`seller-order-pill status-${order.status}`}>{orderStatusLabel(order.status)}</b>
                      <span>{money(order.total || order.subtotal, order.currency)}</span>
              ...
              <section className="seller-panel">
                ...
                <div className="seller-stock-watch">
                  {products.filter(product => product.is_active && Number(product.stock_qty) <= 5).slice(0,6).map(product => (
                    <div key={product.id}>
                      <SmartImage src={productImages[product.id]} fallback="OM" fit="contain" widthHint={120}/>
                      <span><strong>{product.name}</strong><small>{product.stock_qty} en stock</small></span>
                      <button onClick={() => changeTab('products')}>Modifier</button>
                    </div>
                  ))}
            ...
            <div className="seller-product-table">
              ...
              {filteredProducts.map(product => (
                <article key={product.id} className={!product.is_active ? 'inactive' : ''}>
                  ...
                  <div className={`seller-stock-input ${Number(product.stock_qty) <= 5 ? 'low' : ''}`}>
                    <input type="number" min="0" value={product.stock_qty} onChange={event => setProducts(current => current.map(item => item.id === product.id ? { ...item, stock_qty: event.target.value } : item))} onBlur={() => updateProduct(product.id, { stock_qty: Math.max(0, Number(product.stock_qty) || 0) })}/><span>unités</span>
                  </div>
        ...
        {!workspaceLoading && tab === 'orders' && (
          <section className="seller-panel seller-orders-panel">
                ...
                return (
                  <article className="seller-order-card" key={order.id}>
                    ...
                    <div className="seller-order-lines">
                      {lines.map(line => (
                        <div key={line.id}><SmartImage src={line.product_image_url} fallback="OM" fit="contain" widthHint={120}/><span><strong>{line.product_name}</strong><small>Qté {line.quantity}{Object.keys(line.variant_snapshot || {}).length ? ` · ${Object.values(line.variant_snapshot).join(' · ')}` : ''}</small></span><b>{money(line.line_total, line.currency)}</b></div>
                      ))}
                    </div>
                    {parent?.customer_note && <div className="seller-customer-note"><strong>Note du client</strong><span>{parent.customer_note}</span></div>}
                    <div className="seller-order-actions">{next && <button className="button primary" onClick={() => advanceOrder(order)}>{nextStatusAction(order.status)}</button>}{['pending','confirmed'].includes(order.status) && <button className="button secondary danger-outline" onClick={() => refuseOrder(order)}>Refuser</button>}</div>
                  </article>
                )
              })}
        ...
        {!workspaceLoading && tab === 'store' && (
          <section className="seller-panel seller-store-settings">
