...
export default function Header() {
  ...
  const mobileNav = (
    <nav className="mobile-bottom-nav" aria-label="Navigation mobile">
      <NavLink to="/" end><Home size={23}/><span>Accueil</span></NavLink>
      <NavLink to={user ? '/account' : '/auth'}><UserRound size={23}/><span>Compte</span></NavLink>
      <NavLink className="mobile-bottom-cart" to={user ? '/cart' : '/auth'}><span><ShoppingCart size={25}/>{count > 0 && <b>{count > 99 ? '99+' : count}</b>}</span><em>Panier</em></NavLink>
      <NavLink to={user ? '/orders' : '/auth'}><Package size={23}/><span>Commandes</span></NavLink>
      <button className={open ? 'active' : ''} onClick={() => setOpen(value => !value)} aria-label="Menu">{open ? <X size={23}/> : <Menu size={23}/>}<span>Menu</span></button>
    </nav>
  )
      ...
      <header className="site-header market-header">
        <div className="market-topbar"><div className="market-topbar-inner">
          ...
          <div className="nav-actions market-actions">
            ...
            <Link className="market-orders desktop-orders" to={user ? '/orders' : '/auth'}><Package size={20}/><span><small>Suivi</small><strong>Commandes</strong></span></Link>
